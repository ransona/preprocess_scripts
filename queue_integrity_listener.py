...
# the number of experiments to try to check the integrity of in advance
exp_buffer_size = 3

try:
    matrix_msg.main('adamranson','Integrity queue restarted')
    matrix_msg.main('adamranson','Integrity queue restarted','Server queue notifications')
except:
...
while True:
    ...
    if len(files) > 0:
        ...
        # Cycle through the top 3 queued items trying to verify file integrity and as a 
        # byproduct caching the files
        if len(files) > exp_buffer_size:
            files_to_check = range(exp_buffer_size)
        else:
            files_to_check = range(len(files))
        
        for iExp in files_to_check: 
            try:
                start_time = time.time()
                
                # Search for 'file_check*' files and if they exist run file_check_verify using them
                # Then move to the next 

                with open(os.path.join(queue_path,files_sorted[0]), "rb") as file: 
                    queued_command = pickle.load(file)

                print('Starting to check integrity for command:')
