...
class MyWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.data_type_combo.addItems(['dF', 'F', 'Spikes'])                  
        ...
        ch_layout.addWidget(self.data_type_combo)
    ...
    def update_cell_display(self):
        ...
        if self.best_max_check.isChecked(): # and
            ...
            if not trial_indices.empty:
                # if there are trials
                ax.plot(self.data['s2p_cut']['t'],np.transpose(self.data['s2p_cut']['neural'][self.meta['current_cell'],trial_indices,:]),color='lightgray')
                # plot the mean response
                mean_response = np.mean(np.transpose(self.data['s2p_cut']['neural'][self.meta['current_cell'],trial_indices,:]),axis=1)
                ax.plot(self.data['s2p_cut']['t'],mean_response,color='black')
            ...
            for i, stim_id in enumerate(self.meta['current_cond']):
                ...
                if not trial_indices.empty:
                    # if there are trials
                    ax[i].plot(self.data['s2p_cut']['t'],np.transpose(self.data['s2p_cut']['neural'][self.meta['current_cell'],trial_indices,:]),color='lightgray')
                    # plot the mean response
                    mean_response = np.mean(np.transpose(self.data['s2p_cut']['neural'][self.meta['current_cell'],trial_indices,:]),axis=1)
                    ax[i].plot(self.data['s2p_cut']['t'],mean_response,color='black')
                    if len(self.meta['stim_labels'])>=stim_id:
                        # label the plot if label is available
    ...
    def load_file(self):
        ...
        try:
            self.datatype_to_load = self.data_type_combo.currentText()
            neural_data_path = os.path.join(exp_dir_processed_cut,'s2p_ch'+ch+'_' + self.datatype_to_load + '_cut.pickle')
            with open(neural_data_path, "rb") as file: self.data['s2p_cut'] = pickle.load(file)
            # make whatever data was loaded go to the dF field
            if self.datatype_to_load == 'dF':
                    self.data['s2p_cut']['neural'] = self.data['s2p_cut']['dF']
            elif self.datatype_to_load == 'F':
                    self.data['s2p_cut']['neural'] = self.data['s2p_cut']['F']
            elif self.datatype_to_load == 'Spikes':
                    self.data['s2p_cut']['neural'] = self.data['s2p_cut']['spks']
            ...
            self.meta['total_cells'] = self.data['s2p_cut']['neural'].shape[0]
    ...
    def calc_time_averaged(self):
        ...
        start_sample = np.argmax(self.data['s2p_cut']['t']>= self.meta['window_start'])
        end_sample = np.argmax(self.data['s2p_cut']['t']>= self.meta['window_end'])
        # calculate keeping trials seperate
        time_av_resp = np.mean(self.data['s2p_cut']['neural'][:,:,start_sample:end_sample],axis=2)
    ...
    def calc_time_averaged_heat(self):
        ...
        # average over trials of same trialID
        all_stim_ids = np.unique(self.data['all_trials']['stim'])
        trial_av_resp_heat = np.zeros([self.meta['total_cells'],len(self.data['s2p_cut']['t']),len(all_stim_ids)])
        for iStimID in all_stim_ids:
            trial_indices = self.data['all_trials'].loc[(self.data['all_trials']['stim'] == iStimID)].index
            stim_heat = self.data['s2p_cut']['neural'][:,trial_indices,:]
    ...
    def show_heat(self):
        ...
        for i, stim_id in enumerate(self.meta['current_cond']):
            ...
            if not trial_indices.empty:
                ...
                start_time = self.data['s2p_cut']['t'][0]
                end_time = self.data['s2p_cut']['t'][index]
