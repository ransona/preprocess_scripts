...
def run_trial_average_batch(trial_average_config):
    ...
    for expID in expIDs:
        ...
        for ch in chs:
            ...
            neural_activity['dF_pre_mean_trials'] = np.nanmean(data['s2p_dF_cut']['dF'][:,:,pre_samples],2)
            neural_activity['F_pre_mean_trials'] = np.nanmean(data['s2p_F_cut']['F'][:,:,pre_samples],2)
            neural_activity['spikes_pre_mean_trials'] = np.nanmean(data['s2p_spikes_cut']['Spikes'][:,:,pre_samples],2)
            neural_activity['dF_post_mean_trials'] = np.nanmean(data['s2p_dF_cut']['dF'][:,:,post_samples],2)
            neural_activity['F_post_mean_trials'] = np.nanmean(data['s2p_F_cut']['F'][:,:,post_samples],2)
            neural_activity['spikes_post_mean_trials'] = np.nanmean(data['s2p_spikes_cut']['Spikes'][:,:,post_samples],2)
            # calc medians for this time window
            neural_activity['dF_pre_median_trials'] = np.nanmedian(data['s2p_dF_cut']['dF'][:,:,pre_samples],2)
            neural_activity['F_pre_median_trials'] = np.nanmedian(data['s2p_F_cut']['F'][:,:,pre_samples],2)
            neural_activity['spikes_pre_median_trials'] = np.nanmedian(data['s2p_spikes_cut']['Spikes'][:,:,pre_samples],2)
            neural_activity['dF_post_median_trials'] = np.nanmedian(data['s2p_dF_cut']['dF'][:,:,post_samples],2)
            neural_activity['F_post_median_trials'] = np.nanmedian(data['s2p_F_cut']['F'][:,:,post_samples],2)
            neural_activity['spikes_post_median_trials'] = np.nanmedian(data['s2p_spikes_cut']['Spikes'][:,:,post_samples],2)
